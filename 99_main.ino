...
#define PTIME 0xFF // 2.73-ms time units
#define PDRIVE 0 // the drive current 100 mA, 50 mA, 25 mA, or 12.5 mA
#define PDL 0 // Proximity drive level. When asserted, the PDRIVE current is reduced by a factor of 9.
#define PGAIN 0 // Prox gain, the integration rate, 2 bits     1x, 2x, 4x, or 8x
#define PPULSE 0 // the signal increases proportionally to PPULSE, while noise increases by the square root of PPULSE.
#define PDIODE // The proximity diode selector
#define POFFSET 0 // shift proximity data positive or negative
...
void loop() {
  ...
  int lux = (int) calculateLux(ch0, ch1);
  ...
  //analogWrite(10, ch1A);
  //analogWrite(9, luxA);
  unsigned int aGainInput = analogRead(A3);
  ...
  display_resetCursor();
  display_println("ch0", String(ch0, HEX), "ch1", String(ch1, HEX));
  display_println("aGain", String(aGain, DEC));
  display_println("lux", String(lux, DEC));
